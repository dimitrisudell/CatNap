/* Declares the CatNap Audio Worklet Node */

class CatNapController extends WAMController
{
  constructor (actx, options) {
    ...
    super(actx, "CatNap", options);
  }

  static importScripts (actx) {
    var origin = "https://dimitrisudell.github.io/CatNap/";

    return new Promise( (resolve) => {
      actx.audioWorklet.addModule(origin + "scripts/CatNap-wam.js").then(() => {
      actx.audioWorklet.addModule(origin + "scripts/wam-processor.js").then(() => {
      actx.audioWorklet.addModule(origin + "scripts/CatNap-awp.js").then(() => {
        resolve();
      }) }) });
    })
  }
