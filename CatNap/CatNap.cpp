...
CatNap::CatNap(const InstanceInfo& info)
...
//    pGraphics->AttachControl(new ISVGKnobControl(masterArea.GetCentredInside(100), knobSVG, kParamGain)); /* TASK_02 */
    ...
    pGraphics->AttachControl(new IVLabelControl(logoArea, "CatNap", DEFAULT_STYLE.WithDrawFrame(false).WithValueText(IText(50., "Logo"))));
...
#if IPLUG_DSP
void CatNap::ProcessBlock(sample** inputs, sample** outputs, int nFrames)
{
...
void CatNap::ProcessMidiMsg(const IMidiMsg& msg)
{
...
void CatNap::OnParamChange(int paramIdx)
{
