...
def main():
  ...
  if not zip:
    installer = "\\build-win\\installer\\CatNap Installer.exe"
    
    if demo:
      installer = "\\build-win\\installer\\CatNap Demo Installer.exe"
    
    files = [
      projectpath + installer,
      projectpath + "\installer\changelog.txt",
      projectpath + "\installer\known-issues.txt",
      projectpath + "\manual\CatNap manual.pdf" 
    ]
  else:
    files = [
      projectpath + "\\build-win\CatNap.vst3\Contents\\x86_64-win\CatNap.vst3",
      projectpath + "\\build-win\CatNap_x64.exe"  
    ]

  zipname = "CatNap-v" + config['FULL_VER_STR']
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\CatNap-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\CatNap-app_x64.pdb"  
  ]
